...
bool AlefLoginServerList::processPacket(AlefSocket& sock, AlefPacket* packet)
{
	...
	/*1, 32, 1, 1*/
	pktInterface->processPacket(packet, &i8Operation, unkChar, &i16Status, encodedWorld);

	//switch (packet->GetPacketFlag(FlagIndex::FLAG_IDX0)) //Opcode2
	switch (i8Operation) //Opcode2
	{
		case 3: //ServerList Request Packet Opcode 0x18 0x15 0x00
...
bool AlefLoginServerList::processServerList(AlefSocket& sock, AlefPacket* packet)
{
	...
	AlefPacket* serverList = pktInterface->buildPacket(Alef::AGPMWORLD_PACKET_TYPE, 1, &i8Operation, 0, 0, &serverListStrLen, serverListStr);
