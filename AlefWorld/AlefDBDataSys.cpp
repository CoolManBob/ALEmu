...
SharedPtr<RecordSet> AlefDBDataSys::dbGetTemplateData(int tID)
{
	try
	{
		...
		templateData << "SELECT * FROM chartemplate_data WHERE template_id = ?", use(tID), now;
...
SharedPtr<RecordSet> AlefDBDataSys::dbGetLoginCharPos()
{
	try
	{
		Session dbSession = dbInterface->getDataDB();

		Statement posData(dbSession);

		posData << "SELECT * FROM logincharpos", now;

		SharedPtr<RecordSet> rs = new RecordSet(posData);
